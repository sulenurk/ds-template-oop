...
# === 1. LEARN ===
def learn_model(X, y, params=None, search=True, cv=5, scoring='neg_root_mean_squared_error'):
    """Train KNN Regressor with optional hyperparameter tuning"""
...
def apply_model(X, model_info: Dict) -> pd.DataFrame:
    """Apply KNN regressor and return predictions"""
...
def explain_model(model, X_train, X_test, top_n_features=10, sample_index=None, index_feature=False, save_path=None):
    """Use SHAP to explain a KNN regression model"""
    shap_vals = sp.shap_values(model, X_train, X_test, model_type='knn')
