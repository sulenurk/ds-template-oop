...
def learn_model(df, target_col, params=None, search=True, cv=5, scoring='recall', random_state=42):
    """
    Train a Random Forest classifier with optional hyper-parameter tuning.
    ...
    params : dict, optional
        ...
        If ``None``, a default grid is used for tuning.
    ...
    random_state : int, default=42
        Seed passed to ``RandomForestClassifier`` for reproducibility.
...
def apply_model(df, model_info: Dict, columns: Optional[List[str]] = None) -> pd.DataFrame:
    """
    Apply a trained Random Forest model to a DataFrame and append predictions.
    ...
    model_info : dict
        Dictionary with key ``'model'`` mapping to a fitted
